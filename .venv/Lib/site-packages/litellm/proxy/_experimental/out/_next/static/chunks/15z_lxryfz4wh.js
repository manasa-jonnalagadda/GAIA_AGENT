...
client = openai.OpenAI(
	api_key="${y||"YOUR_LITELLM_API_KEY"}",
	base_url="${_}"
)`;switch(g){case i.CHAT:{let e=Object.keys(z).length>0,o="";if(e){let e=JSON.stringify({metadata:z},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();o=`,
    extra_body=${e}`}let r=S.length>0?S:[{role:"user",content:k}];t=`
import base64
...
`;break}case i.RESPONSES:{let e=Object.keys(z).length>0,o="";if(e){let e=JSON.stringify({metadata:z},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();o=`,
    extra_body=${e}`}let r=S.length>0?S:[{role:"user",content:k}];t=`
import base64
